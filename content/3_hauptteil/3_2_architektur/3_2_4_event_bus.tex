...
\begin{figure}[!ht]
	...
	\caption{Sequenzdiagramm der Deregistrierung eines Empfängers beim Eventbus. Unter Verwendung des zuvor erhaltenen \emph{Handles} deregistiert der \code{EventConsumer} die referenzierte \emph{Callback}-Funktion.}
...
\begin{figure}[!ht]
	...
	\caption{Sequenzdiagramm der ersten Variante des Eventbus. Eine Komponente sendet ein Ereignis an den \emph{Logger} (1). Dieser erzeugt ein \code{LogEntry}-Objekt (2)auf welchem \code{save} aufgerufen wird (3). Dabei wirdder Eventbus benachrichtigt (4), welcher die registrierten Empfänger über das neue Ereignis informiert (5).}
...
\begin{figure}[!ht]
	...
	\caption{Klassendiagramm der zweiten Variante des Eventbus. Die Komponenten benachrichtigen den Eventbus direkt über neue Ereignisse. Der Eventbus leitet die Ereignisse dann an die registrierten Empfänger weiter. Die ereignisspezifischen Informationen sind in \code{Event}-Objekten gekapselt.}
